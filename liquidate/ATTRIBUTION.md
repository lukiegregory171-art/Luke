...
| **Weapons** — `rifle` (blaster-repeater) / `pistol` (blaster) (`client/assets/raw/weapons/*` → `client/public/assets/weapons/*`) | Real weapon models, STAGED but currently disabled (`WEAPON_GLB` is empty) so all 7 guns share the procedural style; re-enable per-archetype once a full pack is reachable | [KenneyNL/Starter-Kit-FPS](https://github.com/KenneyNL/Starter-Kit-FPS) | [Kenney](https://kenney.nl) | **MIT** |
